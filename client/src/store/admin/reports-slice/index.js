...
export const generateUserActivityReport = createAsyncThunk(
  "adminReports/generateUserActivityReport",
  async ({ days }, { rejectWithValue }) => {
    try {
      const response = await axios.get(`/api/admin/reports/user-activity?days=${days}`);
...
export const generateSalesTrendReport = createAsyncThunk(
  "adminReports/generateSalesTrendReport",
  async ({ days }, { rejectWithValue }) => {
    try {
      const response = await axios.get(`/api/admin/reports/sales-trend?days=${days}`);
      return response.data.data;
    } catch (error) {
