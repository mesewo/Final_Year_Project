...
export default function StorekeeperProductRequests() {
  ...
  const groupedRequests = requests.reduce((acc, request) => {
    ...
    acc[sellerId].requests.push(request);
    return acc;
  }, {});

  // Get unique sellers for filter dropdown
  const uniqueSellers = Object.values(groupedRequests).map(group => group.seller);
  ...
  .filter(group => {
    // Filter by seller
    if (sellerFilter !== "all" && group.seller._id !== sellerFilter) return false;
